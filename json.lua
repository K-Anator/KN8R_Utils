...
function json.writeJson(path, data)
	local success, error = FS.CreateDirectory(FS.GetParentFolder(path))

	if not success then
		print('failed to create directory for file "' .. tostring(path) .. '", error: ' .. tostring(error),"WARN")
		return false, error
	end
...
function json.copyFile(path_src, path_dst)
